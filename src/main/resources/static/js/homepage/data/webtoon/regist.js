...
ACTIONS = {
                ...
                if (res != null) {
                    let pdfname = `
                        <div  id="file_${res.id}">
                            ...
                            <span>${res.orgFilename}</span>
                            <span>
                                <a href="javascript:void(0)" onclick="ACTIONS.DEL_PDF(${res.id})">X</a>
                            </span>
                        </div>
                    `;
                ...
                if (res != null) {
                    let filename = `
                        <div id="file_${res.id}">
                            ...
                            <span>${res.orgFilename}</span>
                            <span>
                                <a href="javascript:void(0)" onclick="ACTIONS.DEL_PREVIEW(${res.id})">X</a>
                            </span>
                        </div>
                    `;
        ...
        if (fileArr.length > 0) {
            ...
            fileList.forEach(file => {
                ...
                if (type == "pdf") {
                    ...
                    if (ACTIONS.PDF_SIZE_CHECK(file) === false) {
                    ...
                    if (ACTIONS.PREVIEW_SIZE_CHECK(file) === false) {
    ...
    PDF_SIZE_CHECK: function (file) {
        if (file.size) {
            let maxSize = 30 * 1024 * 1024;
            return file.size <= maxSize;
        }
    },
    PREVIEW_SIZE_CHECK: function (file) {
        if (file.size) {
            let maxSize = 30 * 1024 * 1024;
            return file.size <= maxSize;
        }
