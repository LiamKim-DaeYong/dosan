var pageObj = {
    ...
    draft: function (id) {
        $ajax.put({
            data: {approvalType : 'DRAFT'},
            url: `/educator/business-trip-report/${id}/draft`,
            success: function (id) {
                alert("기안되었습니다.");
                location.href = location.href = `/educator/business-trip-report/${id}/detail`;
            }
        })
    },
...
pageObj.pageStart = function () {
    $("select[class^=city]").each(function () {
        var $city = $(this);
        $.each(eval(area0), function() {
            $city.append("<option value='"+(this == '시/도 선택' ? '' : this) +"'>"+this+"</option>");
        });

        var fromOrTo = this.className.split("_")[1];
        $(".county_"+fromOrTo).append("<option value=''>구/군 선택</option>");
    });

    $("select[class^=city]").change(function() {
        var fromOrTo = this.className.split("_")[1];
        var area = "area"+$("option",$(this)).index($("option:selected",$(this)));
        var $county = $('.county_'+fromOrTo);
        $("option", $county).remove();

        if(area == "area0")
            $county.append("<option value=''>구/군 선택</option>");
        else {
            $.each(eval(area), function() {
                $county.append("<option value='"+this+"'>"+this+"</option>");
            });
        }
    });
}
